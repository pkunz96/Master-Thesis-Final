...
def create_contrastive_loss(labels2, margin: float = 1.0) -> Callable[[tf.Tensor, tf.Tensor, tf.Tensor], tf.Tensor]:
    ...
    def contrastive_loss(out_val: tf.Tensor, x: tf.Tensor, x_index: tf.Tensor, y: tf.Tensor) -> tf.Tensor:
        ...
        # Parallelize label retrieval using ThreadPoolExecutor
...
def training_step(first_layer: Layer, x_data: tf.Tensor, y_data: tf.Tensor, optimizer: tf.keras.optimizers.Optimizer, batch_size: int = 5) -> Tuple[Layer, tf.Tensor]:
    ...
    with tf.GradientTape(persistent=False) as tape:
        # Watch all layers' weights and biases
        layer_params = []
        current_layer = first_layer
        while current_layer is not None:
            tape.watch(current_layer.w1)  # Watch weights
            tape.watch(current_layer.b1)  # Watch bias
        ...
        print("")
    """
    # Compute gradients for all layers' parameters
    ...
    for index in range(0, len(dw_list)):
        #dw_list[index].assign_sub(learning_rate * dw_gradient[index])
        dw_list[index].assign_sub(dw_gradient[index]*2)

    for index in range(0, len(db_list)):
        #db_list[index].assign_sub(learning_rate * db_gradient[index])
        db_list[index].assign_sub(db_gradient[index]*2)"""
...
extractor: List[Layer] = [
    Layer(name="in", input_dim=10, output_dim=40, loss_function=None, loss_weight=1.0, activation_function_name="relu"),
    Layer(name="latent", input_dim=40, output_dim=2, loss_function=categorical_cross_entropy_loss, loss_weight=1.0, activation_function_name="softmax"),
]
...
first_layer: Layer = train(compile_model(model), x_data, y_data, epochs=1000, learning_rate=1)
