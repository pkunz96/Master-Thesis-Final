...
from nn.multi_domain_nn_forwarding_v2 import Layer, Procedure, categorical_cross_entropy_loss, binary_representation_loss, \
    create_contrastive_loss, BinaryLayer, AbstractSearch, BayesianSearch, Hyperparameters
...
class BinaryEmbeddingSearch(BayesianSearch):
    ...
    def __init__(self):
        super().__init__(BinaryEmbeddingSearch.training_parameter_list, BinaryEmbeddingSearch.validation_parameter_list, BinaryEmbeddingSearch.test_parameter_list, BinaryEmbeddingSearch.base_dir, 2000, gen_insertion_sort_environment, straight_insertion_sort)
    ...
    def _create_subsequent_procedure_builders(self, hyperparameters: Hyperparameters, training_data_dict: Dict[str, Tuple[tf.Tensor, tf.Tensor]], validation_data_dict: Dict[str, Tuple[tf.Tensor, tf.Tensor]], test_data_dict: Dict[str, Tuple[tf.Tensor, tf.Tensor]]) -> List[Callable[[Procedure], Procedure]]:
        ...
        def fine_tuning(procedure: Procedure):
            ...
            if last_extractor_layer is not None:
                ...
                x_data, y_data = Procedure.merge_training_data(list(training_data_dict.values()))
