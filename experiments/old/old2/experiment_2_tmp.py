...
from nn.multi_domain_nn_forwarding_v2 import Layer, Procedure, categorical_cross_entropy_loss, binary_representation_loss
...
extractor: List[Layer] = [
    ...
    Layer(name="extractor_out", input_dim=128, output_dim=8, forward=True, loss_function=binary_representation_loss, loss_weight=1.0,activation_function_name="relu")
]
...
procedure: Procedure = Procedure(Procedure.compile_model(model), training_data_dict, validation_data_dict, validation_data_dict, 1000, 0.0001, 128, "adam")
procedure.train()

#procedure: MLDGProcedure = MLDGProcedure(Procedure.compile_model(model), training_data_dict, validation_data_dict, validation_data_dict, 2000, 0.0001, 128, "adam", gamma=.2, beta=.2)
#procedure.train()
